...
const byte topEncI    = PE_7;
const byte topHallA   = PA8;
const byte topHallB   = PA9;
const byte topHallC   = PA10;
const byte topMotA    = PD_12;
const byte topMotB    = PD_13;
const byte topMotC    = PD_14;
const byte topMotEn   = PD_9;
const byte topMotnRST = PD_10;
const byte topMotnFlt = PD_11;
#define TOP_MOTOR_CMD 'T'

//      Middle
const byte midCSA     = PB1;
const byte midCSB     = PE_9;
const byte midEncA    = PA15;
const byte midEncB    = PD_4;
const byte midEncI    = PD_3;
const byte midHallA   = PA15;
const byte midHallB   = PD_4;
const byte midHallC   = PD_7;
const byte midMotA    = PB2;
const byte midMotB    = PC_12;
const byte midMotC    = PE_8;
const byte midMotEn   = PC_1;
const byte midMotnRST = PC_2;
const byte midMotnFlt = PC_3;
#define MID_MOTOR_CMD 'M'

//      Bottom
const byte botCSA     = PE_14;
const byte botCSB     = PE_15;
const byte botEncA    = PE_2;
const byte botEncB    = PE_3;
const byte botEncI    = PD_2;
const byte botHallA   = PE_2;
const byte botHallB   = PE_6; // PE3 on Houston-A1 base
const byte botHallC   = PC_0; // PE4 on Houston-A1 base
const byte botMotA    = PC6;
const byte botMotB    = PC_7;
const byte botMotC    = PC_8;
const byte botMotEn   = PE_10;
const byte botMotnRST = PE_11;
const byte botMotnFlt = PE_12;
...
// auto filterTop = butter<2>(f_n);
// auto filterMid = butter<2>(f_n);
// auto filterBot = butter<2>(f_n);
////

#define NUM_STAGES 1 // 2nd order
#define BLOCK_SIZE 1 // one sample per loop
const float32_t coeffs[5] = { // calculated using scipy with f_s=1kHz and f_c=10Hz (I think it expects negative a's)
    0.00094469f, 0.00188938f, 0.00094469f, 1.91119707f, -0.91497583f // 10Hz
    // 0.00157611f, 0.00315221f, 0.00157611f, 1.88460294f, -0.89090736f // 13Hz
    // 0.00208057f, 0.00416113f, 0.00208057f, 1.86689228f, -0.87521455f // 15Hz
};
arm_biquad_cascade_df2T_instance_f32 topFilter;
arm_biquad_cascade_df2T_instance_f32 midFilter;
arm_biquad_cascade_df2T_instance_f32 botFilter;
float32_t state_top[4 * NUM_STAGES] = {0};
float32_t state_mid[4 * NUM_STAGES] = {0};
float32_t state_bot[4 * NUM_STAGES] = {0};
...
#define ORBITA_PID_DEF_P        500.0f // Original 500.0f
...
void setup() {
  ...
  arm_biquad_cascade_df2T_init_f32(&topFilter, NUM_STAGES, coeffs, state_top);
  arm_biquad_cascade_df2T_init_f32(&midFilter, NUM_STAGES, coeffs, state_mid);
  arm_biquad_cascade_df2T_init_f32(&botFilter, NUM_STAGES, coeffs, state_bot);
...
void Orbita_loop() {
  ...
  if(orbita_sensor_filtering_idx++ > ORBITA_SENSOR_FILTERING) {
    ...
    // // USING FILTERS LIBRARY (SLOWER)
    // float filtered_top_sensor_target = filterTop(orbita_top_sensor_target);
    // float filtered_mid_sensor_target = filterMid(orbita_mid_sensor_target);
    // float filtered_bot_sensor_target = filterBot(orbita_bot_sensor_target);

    // USING ARM FILTER LIBRARY (FASTER)
    float32_t filtered_top_sensor_target, filtered_mid_sensor_target, filtered_bot_sensor_target;
    arm_biquad_cascade_df2T_f32(&topFilter, &orbita_top_sensor_target, &filtered_top_sensor_target, BLOCK_SIZE);
    arm_biquad_cascade_df2T_f32(&midFilter, &orbita_mid_sensor_target, &filtered_mid_sensor_target, BLOCK_SIZE);
    arm_biquad_cascade_df2T_f32(&botFilter, &orbita_bot_sensor_target, &filtered_bot_sensor_target, BLOCK_SIZE);

    // Setting commands to motors -> command = pid ( target - measurement )
    // 8 us every 1,07 ms (filtering 1/10)
    top_motor.target = Orbita_Top_PID(filtered_top_sensor_target - orbita_top_present_position);
    top_motor.target *= -1.0;
    mid_motor.target = Orbita_Mid_PID(filtered_mid_sensor_target - orbita_mid_present_position);
    mid_motor.target *= -1.0;
    bot_motor.target = Orbita_Bot_PID(filtered_bot_sensor_target - orbita_bot_present_position);
...
void AS5047_init(void) {
  ...
  Serial3.print("Raw top: ");
  Serial3.println(orbita_top_sensor.readRawAngle(), HEX);

  Serial3.print("Raw mid: ");
  Serial3.println(orbita_mid_sensor.readRawAngle(), HEX);

  Serial3.print("Raw bot: ");
  Serial3.println(orbita_bot_sensor.readRawAngle(), HEX);
