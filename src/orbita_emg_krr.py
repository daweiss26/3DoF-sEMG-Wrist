"""
Uses a trained KRR model specified by MODEL_FILE to take in EMG signal from a Mindrove armband.
...
def main():
    ...
    model_krr = joblib.load(MODEL_FILE)
        ...
        try:
            while True:
                ...
                if mindrove.get_board_data_count() >= REQ_SAMPLES:
                    ...
                    emg_norm = adaptive_scaler.update_and_normalize(emg_window)
                    rms_features = np.sqrt(np.mean(np.square(emg_norm), axis=0))
                    input_tensor = rms_features.reshape(1, -1) # KRR expects 2D input (1 sample, 8 features)
                    
                    # Predict
                    prediction = model_krr.predict(input_tensor)[0]
