...
use io::{CachedDynamixelSerialController, DynamixelSerialController};

#[cfg(all(feature = "build_dynamixel", unix))]
use io::{CachedDynamixelPoulpeController, DynamixelPoulpeController};
...
impl Orbita3dController {
    ...
    pub fn enable_torque(&mut self, reset_target: bool) -> Result<()> {
        if reset_target { // !self.is_torque_on()? &&
