// In this and the pouple.rs file, I...
// Replaced DynamixelSerialIO with DynamixelProtocolHandler
// Replaced device::orbita3d_poulpe with servo::orbita::orbita3d_foc
// Added target_cache, returned it on get_target_position, and set it when setting get_target_position
// Repalced write_target_position with write_target_position_fb

use crate::ZeroType;
use motor_toolbox_rs::{Limit, MissingRegisterError, MotorsController, RawMotorsIO, Result, PID};
use rustypot::{
    DynamixelProtocolHandler,
    servo::orbita::orbita3d_foc::{self, DiskValue},
};
...
impl DynamixelSerialController {
    ...
    pub fn new(serial_port: &str, id: u8, zero: ZeroType, reductions: f64) -> Result<Self> {
        log::info!("Opening serial {}", serial_port);
        let mut controller = Self {
            serial_port: serialport::new(serial_port, 1_000_000)
                .timeout(Duration::from_millis(100))
                .open()?,
            io: DynamixelProtocolHandler::v1(),
...
impl RawMotorsIO<3> for DynamixelSerialController {
    ...
    fn is_torque_on(&mut self) -> Result<[bool; 3]> {
        let val: u8 = orbita3d_foc::read_torque_enable(&self.io, self.serial_port.as_mut(), self.id)?;
        let on = val != 0;
        Ok([on; 3])
    }
    ...
    fn set_torque(&mut self, on: [bool; 3]) -> Result<()> {
        ...
        let val: u8 = if on[0] { 1 } else { 0 };
        orbita3d_foc::write_torque_enable(
            &self.io,
    ...
    fn get_current_position(&mut self) -> Result<[f64; 3]> {
        let thetas: DiskValue<f32> =
            orbita3d_foc::read_present_position(&self.io, self.serial_port.as_mut(), self.id)?;
        Ok([
            thetas.top as f64,
...
mod tests {
    ...
    fn parse_config_file() {
        ...
        if let Orbita3dIOConfig::DynamixelSerial(dxl_config) = config.io {
            ...
            assert_eq!(dxl_config.serial_port, "/dev/cu.usbserial-B003LPN0");
