...
use rustypot::{
    ...
    servo::orbita::orbita3d_poulpe::{self, MotorValue},
};
...
impl DynamixelPoulpeController {
    ...
    pub fn new(serial_port: &str, id: u8, zero: ZeroType, reductions: f64) -> Result<Self> {
        let mut controller = Self {
            serial_port: Box::new(
                serialport::new(serial_port, 1_000_000)
                    .timeout(Duration::from_millis(10))
                    .open_native()?,
            ),
            // io: DynamixelProtocolHandler::v1(),
            io: DynamixelProtocolHandler::v1().with_post_delay(Duration::from_micros(1000)),
            id,
            offsets: [None; 3],
...
impl RawMotorsIO<3> for DynamixelPoulpeController {
    ...
    fn set_target_position(&mut self, position: [f64; 3]) -> Result<()> {
        let fb = orbita3d_poulpe::write_target_position_fb(
            &self.io,
            self.serial_port.as_mut(),
            self.id,
            MotorValue {
